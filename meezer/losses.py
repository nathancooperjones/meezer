...
def get_hinge_losses():
    hinge_losses = set(
        [keras.losses.hinge, keras.losses.squared_hinge, keras.losses.categorical_hinge]
    )
    return hinge_losses


def get_categorical_losses():
    categorical_losses = set(
        [
...
def _cosine_distance(x, y):
    return tf.losses.cosine_distance(
        tf.nn.l2_normalize(x, 0), tf.nn.l2_normalize(y, 0), axis=0
    )
...
def pn_loss(margin=1):
    def _pn_loss(y_true, y_pred):
        ...
        minimum_distance = K.min(
            K.concatenate([anchor_negative_distance, positive_negative_distance]),
            axis=-1,
            keepdims=True,
        )

        return K.mean(
            K.maximum(anchor_positive_distance - minimum_distance + margin, 0)
        )
...
def euclidean_loss(margin=1):
    def _euclidean_loss(y_true, y_pred):
        ...
        return K.mean(
            K.maximum(
                _euclidean_distance(anchor, positive)
                - _euclidean_distance(anchor, negative)
                + margin,
                0,
            )
        )
...
def manhattan_loss(margin=1):
    def _manhattan_loss(y_true, y_pred):
        ...
        return K.mean(
            K.maximum(
                _manhattan_distance(anchor, positive)
                - _manhattan_distance(anchor, negative)
                + margin,
                0,
            )
        )
...
def chebyshev_loss(margin=1):
    def _chebyshev_loss(y_true, y_pred):
        ...
        return K.mean(
            K.maximum(
                _chebyshev_distance(anchor, positive)
                - _chebyshev_distance(anchor, negative)
                + margin,
                0,
            )
        )
...
def cosine_loss(margin=1):
    def _cosine_loss(y_true, y_pred):
        ...
        return K.mean(
            K.maximum(
                _cosine_distance(anchor, positive)
                - _cosine_distance(anchor, negative)
                + margin,
                0,
            )
        )
...
def cosine_pn_loss(margin=1):
    def _pn_loss(y_true, y_pred):
        ...
        minimum_distance = K.min(
            tf.stack([anchor_negative_distance, positive_negative_distance]),
            axis=0,
            keepdims=True,
        )
